...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    with st.sidebar:
        ...
        def prompt_text_change():
            text = f"已保存 Prompt 编辑。"
            st.toast(text)

        prompt_template_show = st.text_area(
            "请编辑Prompt：",
            PROMPT_TEMPLATES[index_prompt[dialogue_mode]][st.session_state.prompt_template_select],
            on_change=prompt_text_change,
            key="prompt_template_show",
        )
        # PROMPT_TEMPLATES[index_prompt[prompt_template_name]] = prompt_template_show
