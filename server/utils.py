...
from configs import (LLM_MODELS, LLM_DEVICE, EMBEDDING_DEVICE,
                     ...
                     FSCHAT_MODEL_WORKERS, HTTPX_DEFAULT_TIMEOUT, PROMPT_CONFIG)
import os
...
def modify_prompt_template(type: str, name: str):
    '''
        从prompt_config中修改模板内容
        type: "llm_chat","agent_chat","knowledge_base_chat","search_engine_chat"的其中一种，如果有新功能，应该进行加入。
    '''
    import ast
    with open(PROMPT_CONFIG, 'r') as file:
        source = file.read()
    tree = compile(source, filename=PROMPT_CONFIG, mode='exec')

    class FindDictNodeVisitor(ast.NodeVisitor):
        def visit_Assign(self, node):
            if isinstance(node.targets[0], ast.Name) and node.targets[0].id == 'my_dict':
                self.found_nodes.append(node)

    visitor = FindDictNodeVisitor()
    visitor.generic_visit(tree)
    assign_nodes = visitor.found_nodes

    for assign in assign_nodes:
        # 获取原始字典表达式
        original_expr = assign.value

        # 创建一个新的字典表达式，并添加/更新键值对
        new_expr = ast.Expr(
            value=ast.BinOp(left=original_expr, op=ast.Add(), right=ast.Constant(value={'new_key': 'new_value'})))

        # 构造新的Assign节点
        new_assign = ast.Assign(targets=[assign.targets[0]], value=new_expr)

        # 将新的Assign节点替换原始的Assign节点
        parent = assign.parent
        index = list(parent).index(assign)
        del parent.body[index]
        parent.body.insert(index, new_assign)

    from astunparse import unparse

    modified_code = unparse(tree)
    with open(PROMPT_CONFIG, 'w') as file:
